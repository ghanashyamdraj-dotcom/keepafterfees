...
export function layout({ site, page, trail, body, css, inlineData = null, hreflang = '', navTools = () => [] }) {
  ...
  /**
   * One sitewide social card. This used to be `/og/${page.slug}.png`, a
   * per-page path the build never wrote — so every page on the site declared a
   * preview image that 404'd, on every Slack, WhatsApp, X and LinkedIn unfurl.
   * Generated by build/og-image.mjs; see there for why it is one card and not
   * thirty-six. If per-page cards are ever added, restore the slug form and
   * make the build write a file for every slug it emits.
   */
