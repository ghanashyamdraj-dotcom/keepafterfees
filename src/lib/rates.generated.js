 ...
 * Generated: 2026-08-24T17:10:37.157Z
 */
...
export const site = {
  ...
  "adSlots": {
    ...
    "slotIds": {
      "leaderboard": "1805655779",
      "result": "",
      "midContent": "",
      "rail": "",
      "endContent": ""
    },
    "slotIdNote": "Populate each of these with the ad unit ID from AdSense. A position with an empty id renders the reserved, correctly-sized container and nothing else, so the layout is already holding the space and filling one in causes no shift. leaderboard carries the horizontal responsive unit created 2026-08-24. The remaining four are still empty ON PURPOSE: they are rectangle (result, midContent, endContent) and vertical (rail) placements, and dropping a horizontal unit into them would ask AdSense for a shape the reserved box was not sized for. Create a rectangle unit and a vertical unit in AdSense and paste their ids here; reusing one id across placements also merges them in reporting, so you could never tell which position was earning.",
    "formats": {
      "leaderboard": "auto",
      "result": "",
      "midContent": "",
    ...
    "formatNote": "Set to the AdSense data-ad-format for a position. A non-empty value marks the unit RESPONSIVE: the container switches from a fixed box to full width with the declared size as a MIN-height, and the unit gets data-ad-format plus data-full-width-responsive. The minimum matters both ways — it still reserves space so the common case shifts nothing, and it never caps the height, because clipping an ad that came back taller than expected is an AdSense policy violation as well as lost revenue. Leave empty for a fixed-size unit, which keeps the exact reserved box."
  },
  "affiliates": {
    "enabled": false,
